...
> **Android 10+ 剪贴板机制**：系统只允许「当前拥有焦点的 App」读取剪贴板，前台 Service **不能**在后台直接读。v1.1.2 起在检测到剪贴板变化时会自动启动**透明 Activity** 短暂获取焦点后读取并派发（无动画、不进最近任务）。若某厂商 ROM 仍拦截后台启动 Activity，可用手动派发作为兜底。
